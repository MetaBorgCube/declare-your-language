...
public class L_A_Class  implements INodeList
{ 
  public INodeSource source;

  public L_A_Class (INodeSource source) 
  { 
    this(source, null, null);
  }

  public L_A_Class (INodeSource source, A_Class head, L_A_Class tail) 
  { 
    this.source = source;
    this.head = head;
    this.tail = tail;
  ...
  @Child public A_Class head;

  @Children public L_A_Class tail;
  ...
  { 
    this.head = (A_Class)newHead;
  }

  @Override public L_A_Class tail()
  { 
    if(tail == null)
    { 
      throw new InterpreterException("No such element exception", null, this);
  ...
  @Override public L_A_Class fromStrategoTerm(IStrategoTerm alist)
  { 
    L_A_Class list = new L_A_Class(NodeSource.fromStrategoTerm(alist));
    for(int l = alist.getSubtermCount() - 1; l >= 0; l--)
    { 
      final INodeSource source = NodeSource.fromStrategoTerm(alist.getSubterm(l));
      list = new L_A_Class(NodeSource.fromStrategoTerm(alist), new Generic_A_Class(source, alist.getSubterm(l)), list);
    }
    return list;
  }
}