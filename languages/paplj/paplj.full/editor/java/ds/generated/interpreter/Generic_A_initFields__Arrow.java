...
{ 
  ...
  @Override public A_initFields__Arrow specialize(int depth)
  { 
    if(replaced())
    { 
      return (A_initFields__Arrow)replacement();
    }
    ...
    { 
      ...
      if(name.equals("initFields") && term.getSubtermCount() == 1)
      { 
        A_initFields__Arrow replacement = replace(new initFields_1(source, new L_A_Field(NodeSource.fromStrategoTerm(term.getSubterm(0))).fromStrategoTerm(term.getSubterm(0))));
        if(depth > 0)
        { 
